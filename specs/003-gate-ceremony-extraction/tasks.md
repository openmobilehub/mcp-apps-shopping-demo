...
- [ ] T014 [US1] Demo consumes `attesto.mount()` for this rail: the demo checkout page links to the credential routes; remove the demo's storefront-local age stub (supersedes `feat/storefront-age-enforcement`; keep the `place-order` 403, move verification ownership to the gate). **PACKAGE-COMPOSITION HALF DONE (feat/003-storefront-gate-compose):** `@openmobilehub/attesto-storefront` publishes the ceremony seams on `app.locals.attesto` and `attesto.mount(store.app)` wires the credential rail; the storefront checkout page links `requires[].approveUrl` → `/attesto/credential?order=…&cred=age|membership`, a buyer proves age + presents membership over HTTP, and the shared `completeOrder` enforces the age gate (e2e + bypass tests in `storefront-gate.test.ts`). **REMAINING:** rewire the root demo (`server.ts`/`app.ts`) to consume `mount()` and drop its storefront-local age stub.
...
- [ ] T019 [US2] Demo consumes the passkey route via `mount()`; optional Hedera/x402 settlement through the injected `settlement` seam (demo-mode; failed settlement ⇒ no paid record, FR-013). **PACKAGE-COMPOSITION HALF DONE (feat/003-storefront-gate-compose):** `attesto.mount(store.app)` wires the passkey rail onto the storefront (`/attesto/passkey?order=…` mounted + served, confirmed live over HTTP), and `createStorefront({ settle })` threads an optional settlement seam into the shared `completeOrder` (a thrown settle records nothing). **REMAINING:** rewire the root demo to consume the passkey route via `mount()`; the storefront e2e authorizes payment via the dc-payment rail (a real WebAuthn assertion needs a virtual authenticator, out of scope for the supertest e2e).
...
- [ ] T023 [US3] Demo consumes the dc-payment route via `mount()`. **PACKAGE-COMPOSITION HALF DONE (feat/003-storefront-gate-compose):** `attesto.mount(store.app)` wires the dc-payment rail onto the storefront (`/attesto/dc-payment?order=…`); the storefront e2e authorizes payment through it end-to-end (presence-only), completing via the SHARED `completeOrder` with the discounted amount bound (line sum == total == bound amount) and the order-status poll reflecting completion. **REMAINING:** rewire the root demo to consume the dc-payment route via `mount()`.
...
- [X] T031 [US1/US3] Re-extract the REAL credential/payment crypto (no scaffold shortcut). The credential-gate + dc-payment rails' OpenID4VP/org-iso-mdoc paths were faithfully ported from the demo's `payment-gate/` (source only — the demo is unmodified): a shared `src/ceremony/mdoc/` module (`mdoc.ts` ISO 18013-5 CBOR DeviceResponse parse; `mdoc-iso.ts` iOS org-iso-mdoc wire — canonical CBOR, COSE_Sign1 ReaderAuthAll, `@peculiar/x509` reader-cert chain, HPKE decrypt; `reader.ts` reader cert + ECDH key; `readerContext.ts` JWE-sealed context), the credential rail's real `verifyCredentialPresentation` (jose ECDH-ES JWE decrypt + nonce binding) and `verifyMdocPresentation` (HPKE, origin-bound), and dc-payment's real `verifyDcPresentation` (JWE decrypt → device-signed `transaction_data_hash` re-checked against SHA-256 of the sealed transaction_data → four gates). The instant-demo `claims`/`amount` path is preserved as the tested default so CT3/CT4/CT5/CT9/CT10/CT11 (credential) + CT6/CT7/CT8 (dc-payment) stay green; the `/request` route now returns a REAL ES256-signed OpenID4VP request (not 501). New tests drive the real path (`mdoc/mdoc-iso.test.ts` wire + signature verify + a forged-signature/origin-mismatch rejection; `*/presentation.test.ts` real JWE/HPKE round-trips + tampered-hash / value-bypass / nonce-mismatch rejections). `trust_level` stays `presence-only-demo` — the wire/COSE/x509/JWE/HPKE/nonce crypto is REAL and verified, but the issuer trust anchor is not (main self-signs its mdoc certs), so it is honestly fenced; nothing is advertised as issuer-verified.
...
- [X] T030 [CT13] ONE shared three-gate checkout page: lift `renderRequirements(order, manifest, verification)` into `packages/attesto-gate/src/ceremony/checkout-page.ts` (numbered gates in policy order, payment LAST, live status pending→✓, membership discount in the total, presence-only honesty note; ROUTE-AGNOSTIC — each gate links to its OWN manifest `approveUrl`, so the demo's `/credential-gate/*` and the storefront's `/attesto/*` links both render unchanged), export it from the package index, and render BOTH surfaces through it: the storefront's `GET /checkout` (replacing the bare `<ul>`; drop the "Complete purchase (demo)" bypass on a gated order — completion goes through the fail-closed payment gate; ungated keeps a simple complete path) AND the committed demo's `checkout.ts`/`app.ts` page. The demo's FUNCTIONAL behavior is preserved (payment stays locked until age is verified, the loyalty math, the age 403 on place-order, no cross-order bleed) — every demo SECURITY test (`checkout-gate.test.ts`, `app.test.ts` age-gate/payment-lock/cross-order) stays green UNCHANGED; only HTML-pinning PRESENTATION strings were re-homed to the unified (equivalent) markup (FR-011).
