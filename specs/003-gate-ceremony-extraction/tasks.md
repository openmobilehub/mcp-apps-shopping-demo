...
- [ ] T030 Shared three-gate checkout renderer (CT13): lift the demo's good `renderCheckoutPage` (checkout.ts — numbered gates with status, payment last, discount updates the total) into `@openmobilehub/attesto-gate` as a reusable `renderRequirements(order, manifest, verification)` → HTML that renders from the `requirements()` manifest's approveUrls (route-agnostic). Then BOTH consume it: (a) `packages/attesto-storefront/src/server.ts` checkout page replaces its bare `<ul>` with it; (b) the demo's `checkout.ts` calls it — **behavior-preserving** so `checkout.test.ts`/`app.test.ts`/`checkout-gate.test.ts` stay green (the demo already computes the manifest at `server.ts:433`, so this is a view swap, not a logic rewire). Advances the T014/T019/T023 demo-consume view. Honesty surfaces (FR-011) preserved.
