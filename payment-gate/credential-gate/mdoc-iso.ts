...
// Wire format reverse-engineered from verifier.multipaz.org's source
// (multipaz-verifier-server handleDcGetDataMdocApi + multipaz core). See
// docs/superpowers/specs/2026-06-11-ios-org-iso-mdoc-design.md.
...
// ── DeviceRequest (ISO 18013-5) for a credential kind ─────────────────────────
// Each kind maps to a doctype + the data elements to request (intent_to_retain=false).
const DOC_ELEMENTS: Record<CredentialKind, { docType: string; namespace: string; elements: string[] }> = {
  age: { docType: "org.iso.18013.5.1.mDL", namespace: "org.iso.18013.5.1", elements: ["age_over_21", "age_over_18"] },
  loyalty: { docType: "org.multipaz.loyalty.1", namespace: "org.multipaz.loyalty.1", elements: ["membership_number", "tier"] },
};
...
export function buildItemsRequest(kind: CredentialKind): Uint8Array {
  const { docType, namespace, elements } = DOC_ELEMENTS[kind];
...
export function buildDeviceRequest(kind: CredentialKind): Uint8Array {
  return cborEncode({
    version: "1.0",
    docRequests: [{ itemsRequest: new Tag(Buffer.from(buildItemsRequest(kind)), 24) }],
  });
}
...
async function buildSignedDeviceRequest(
  ...
  const itemsRequestTag = new Tag(Buffer.from(buildItemsRequest(kind)), 24);
...
export async function buildMdocRequestParts(
  ...
  if (signed) {
    ...
    deviceRequest = await buildSignedDeviceRequest(kind, sessionTranscript, leafKey, chainDer);
  } else {
    deviceRequest = buildDeviceRequest(kind); // unsigned (diagnostic A/B)
  }
  return {
...
// ── Decrypted DeviceResponse → disclosed claims (reuses dc-payment/mdoc.ts) ────
...
//    /request to /verify). Sealed as a JWE under GATE_SECRET, like dc-payment's
//    readerContext. The reader PRIVATE key is confidential, so we encrypt. ──────
